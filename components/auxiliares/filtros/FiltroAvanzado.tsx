      ...
      {mostrarFiltros && (
        <div className="mt-4 pt-4 border-t grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {filtrosDisponibles?.ra && (
                <div><label className="text-sm font-medium">Resultados de Aprendizaje</label><select multiple value={filtros.raIds || []} onChange={e => manejarCambioFiltro('raIds', Array.from(e.target.selectedOptions, (o: HTMLOptionElement) => o.value))} className="w-full p-2 border rounded mt-1 bg-white h-24">{/* FIX: Add explicit type to map callback argument to fix type inference issue. */ Object.values(resultadosAprendizaje).map((ra: ResultadoAprendizaje) => <option key={ra.id} value={ra.id}>{ra.nombre}</option>)}</select></div>
            )}
